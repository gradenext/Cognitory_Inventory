...
import { createLesson } from "../../services/courseAPIs";
import { successToast, errorToast } from "../toast/Toast";

const CreateLessonModal = ({ courseId, moduleId, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await createLesson(courseId, moduleId, { ...form, order: Number(form.order) });
      successToast("Lesson created. You can now upload the file.");
      onSuccess();
      onClose();
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-black border border-white/20 rounded-2xl p-6 w-full max-w-md mx-4">
        <h2 className="text-white text-xl font-bold mb-5">Add Lesson</h2>
        <p className="text-white/50 text-sm mb-4">Create the lesson first, then upload the PDF or PPT file.</p>
