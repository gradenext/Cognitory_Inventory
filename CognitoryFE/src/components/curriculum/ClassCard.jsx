...
const ClassCard = ({ data }) => {
  const { name, enterprise, subjects = [], createdAt, _id } = data;
  ...
  return (
    <div className="relative">
      <div className="absolute z-10 right-2 top-2">
        <Update id={_id} type={"class"} />
      </div>
      <Link to={`${_id}`}>
        <div className="bg-white/10 cursor-pointer backdrop-blur-md border border-white/10 rounded-xl text-white p-4 flex flex-col gap-2 shadow-md transition hover:shadow-lg h-fit">
          {/* Avatar + Class Info */}
